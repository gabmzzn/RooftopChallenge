...
function swapKeyByValue(chars) {
	return Object.fromEntries(Object.entries(chars).map(([k, v]) => [v, k]))
}

function getURL(chars, positions) {
	const charArray = []
	const charObjList = swapKeyByValue(chars)

	for (const key in positions) {
		positions[key].forEach(pos => {
			charArray[pos] = charObjList[key]
		})
	}
...
console.log(getURL(chars, positions))
